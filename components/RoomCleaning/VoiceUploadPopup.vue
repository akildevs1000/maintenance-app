  ...
  <v-dialog v-model="dialog" height="300" width="300">
    ...
    <template v-slot:activator="{ on, attrs }">
      <span v-bind="attrs" v-on="on"
        ><v-icon color="purple">mdi-microphone-outline</v-icon></span
      >
    </template>
          ...
          <v-col class="text-center">
            <v-card outlined class="pa-15">
              <span class="primary--text">
                <WidgetsVoice @voice-note="handleFileSelection" />
              </span>
            </v-card>
          </v-col>
        </v-row>
...
export default {
  ...
  computed: {
    fileName() {
      return `${this.item?.room}_${Date.now()}.mp3`;
    },
  },
  methods: {
    ...
    async update() {
      try {
        await this.$axios.post(`upload-voice-note`, {
          id: this.item.id,
